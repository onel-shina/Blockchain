...
public class Block {
    ...
    public synchronized String computeMerkleRoot(ArrayList<String> lstItems) {
        /*
        Using recursion to compute the merkle root.
        If it's the first run, then we need to hash the strings.
        For recursive runs, we do not need to compute the hash of strings, we only need to combine
        them together and them hash them to produce the parent node's hash.
         */
        return computeMerkleRootMain(lstItems, true);
    }

    public synchronized String computeMerkleRootMain(ArrayList<String> lstItems, boolean firstRun) {
        ArrayList<String> tempList = new ArrayList<>();
        if (lstItems.size() == 1) {
            return lstItems.get(0);
        } else {
            for (int i = 0; i < lstItems.size() - 1; i += 2) {
                MerkleNode leftNode = new MerkleNode();
                MerkleNode rightNode = new MerkleNode();
                MerkleNode parentNode = new MerkleNode();

                if (firstRun) {
                    leftNode.sHash = BlockchainUtil.generateHash(lstItems.get(i));
                    rightNode.sHash = BlockchainUtil.generateHash(lstItems.get(i + 1));
                } else {
                    leftNode.sHash = lstItems.get(i);
                    rightNode.sHash = lstItems.get(i + 1);
                }

                populateMerkleNode(parentNode, leftNode, rightNode);

                tempList.add(parentNode.sHash);
            }
            return computeMerkleRootMain(tempList, false);
        }
    }
